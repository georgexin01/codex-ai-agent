...
verification:
  - Scan this checklist on every Vben Admin task before shipping.
  - Fix missing protections as part of the task.
version: 1.0
...
**HTML editor height contract**
- Use the shared `html-content-editor.vue` component for blog content fields.
- Keep the editor inside a scrollable box with a configurable `maxHeight` (500px default).
- When content exceeds the max height, scroll inside the editor instead of growing the drawer/page.
- Apply the same height behavior to both edit mode and read-only display mode.
- Keep the Quill wrapper non-scoped so the global snow CSS stays overrideable.
...
## 9. VIPBillion Soft Delete Standard â€” `isDelete` Must Exist

**Applies to:** VIPBillion admin business tables that are managed from CRUD screens.

**Rules:**
1. New business tables should include `isDelete boolean NOT NULL DEFAULT false` unless the module explicitly uses `deleted_at`
2. List/detail/drawer/options/workflow queries must filter active rows with `isDelete = false`
3. Delete actions must soft-delete by setting `isDelete = true`
4. If direct row updates are blocked by RLS or trigger side effects, create a `SECURITY DEFINER` RPC and call that from the store
5. Relation lookups, picker options, and embedded drawer lists must also exclude deleted rows
...
SOFT DELETE:
[ ] VIPBillion CRUD tables include isDelete boolean default false when the module needs soft delete?
[ ] All list/detail/drawer/options/workflow reads filter isDelete = false?
[ ] Delete actions use soft delete or RPC, not hard delete?
