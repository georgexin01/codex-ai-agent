...
verification:
  - Confirm the type file and Pinia store match the analyzed schema.
  - Verify enum, FK, and option array mappings before moving to views or SQL.
---
...
export const use{{ENTITY_NAME}}sStore = defineStore('{{ENTITIES}}', {
  ...
  actions: {
     ...
     */
    // Slug/archive rule:
    // If the entity has an auto-generated slug/url, archive the old slug on soft delete
    // by appending a short random suffix so future create/update calls can reuse the clean slug.
