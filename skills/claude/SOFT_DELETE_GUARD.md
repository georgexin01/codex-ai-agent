...
title: Soft Delete Guard
description: "Mandatory Vben Admin query rule for soft-deletable tables. Enforces `.is('deleted_at', null)` on every read path, including relation embeds, option lookups, counts, and drawer sub-lists."
aliases:
  - soft delete guard
  - deleted_at guard
  - active rows only
...
verification:
  - Confirm every query path filters out deleted rows.
  - Confirm relation embeds and option lookups also respect deleted_at is null.
---
...
**Every soft-deletable table requires an active-row filter on EVERY query path — including relation embeds, count columns, name lookups, options dropdowns, and drawer sub-lists.**

- Legacy soft-delete tables use `.is('deleted_at', null)`
- VIPBillion admin business tables usually use `.eq('isDelete', false)`
- Soft delete means the row stays in Supabase but becomes invisible to admin CRUD reads
- When a direct row update is fragile under RLS or trigger behavior, use a `SECURITY DEFINER` RPC that flips the soft-delete flag

Soft-deleted = hidden from all display purposes.

For VIPBillion, `isDelete = true` means deleted/hidden, `isDelete = false` means active/alive.
...
## VIPBillion isDelete Pattern

For VIPBillion admin CRUD tables, `isDelete` is the preferred soft-delete field.

### Required rules

- Add `isDelete boolean NOT NULL DEFAULT false` to the table when the entity is managed from the admin panel
- Keep all reads filtered with `isDelete = false`
- Keep relation embeds, dropdowns, drawers, workflows, and counts on active rows only
- Soft delete by setting `isDelete = true`
- Do not hard delete from the frontend for business tables
- If Supabase RLS or trigger behavior makes direct updates unstable, expose a `SECURITY DEFINER` RPC and call that from the store

### VIPBillion examples

```ts
// Main list
supabase.from('driver_jobs')
  .select('*')
  .eq('isDelete', false)

// Dropdown / relation lookup
supabase.from('driver_jobs')
  .select('id, booking_id')
  .eq('isDelete', false)

// Soft delete
supabase.rpc('soft_delete_driver_job', { p_driver_job_id: id })
```
