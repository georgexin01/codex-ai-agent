...
- [ ] Percentage inputs use the shared `%` addon standard with trimmed decimals and theme-aware addon styling
- [ ] Status labels in tables and drawers use theme tokens, not hardcoded gray/green colors
- [ ] Drawer/card shells use `bg-background`, `border-border`, `text-foreground`, and `text-muted-foreground`

## 4. Percentage Input Addon Standardization

All percentage inputs in VIPBillion admin panels should use the same right-side `%` addon design.

### Standard Percentage Input Pattern

```vue
<InputNumber
  class="deposit-percent-input w-full"
  :min="0"
  :max="100"
  :precision="2"
  :formatter="(value) => formatTrimmedDecimal(value)"
  :parser="(value) => parseNumericInput(value)"
>
  <template #addonAfter>
    <span class="deposit-percent-addon">%</span>
  </template>
</InputNumber>
```
...
- Use theme tokens for the addon background and border so white mode and dark mode both look correct
- Keep the `%` text muted, not cyan or bright green
- Keep the addon compact and aligned to the right edge of the input
- Reuse the same class names for every percentage field: `deposit-percent-input` and `deposit-percent-addon`
...
```css
:deep(.deposit-percent-input .ant-input-number-group-addon) {
...
## 5. Theme-Aware White/Dark Surface Standard

Use this when a module needs to look correct in both light and dark themes, especially list columns, detail drawers, and form sections.

### Surface Rules

- Prefer `bg-background`, `border-border`, `text-foreground`, and `text-muted-foreground`
- Avoid hardcoded `bg-[#1f2127]`, `border-gray-700`, `text-gray-400`, or `text-white` in reusable view modules
- Use `rounded-lg border border-border bg-background p-4 shadow-sm` for detail cards and grouped form blocks
- Use `rounded-lg border border-dashed border-border bg-background` for empty image/state placeholders
- Use `text-primary` or theme-aware `dark:` variants only when the status must still look like a success badge in dark mode

### Status Badge Rule

- White mode: active badges should read as a light green success label with clear border and dark text
- Dark mode: the same badge should switch to a darker emerald success style using `dark:` classes
- Inactive/default states should stay neutral with `bg-muted` and `text-muted-foreground`

### HTML Content Rule

- Use the shared `html-content-editor.vue` for long HTML fields
- Set a module-specific `maxHeight` when the drawer should not stretch
- Let the editor scroll internally instead of letting the drawer grow vertically

### Layout Reminder

- The status badge pattern should be applied consistently in view drawers and summary cards
- Table status cells should keep their renderer centralized, while detail views can use explicit theme classes when the shared renderer is not enough
