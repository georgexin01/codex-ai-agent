...
verification:
  - Confirm the generated components match the entity definition and field list.
  - Keep the view layer aligned with the store and route outputs.
---
...
// Default: sort column is default sort DESC → drag starts enabled
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  gridOptions: {
    ...
    sortConfig: {
      remote: true,
      trigger: 'default',
      defaultSort: { field: 'sort', order: 'desc' },
    },
  },
});
```
...
**Automatic enforcement rule:** Any time AI creates, updates, or repairs a VXE table that uses both a `sort` column and a `No./seq` column, this drag-handle behavior must be applied automatically. Do not leave the 6-dot handle always visible on sort-enabled tables.
