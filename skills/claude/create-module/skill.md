...
verification:
  - Confirm the module has SQL, store, views, routes, i18n, and workflow tests before shipping.
  - Use the smallest route-first path when the task is only one stage of the pipeline.
---
...
**VIPBillion admin rule:** when the module is a business CRUD table, treat `isDelete` as a required soft-delete column unless the legacy table already uses `deleted_at`. All admin list/detail/drawer/dropdown queries must filter active rows, and delete actions should flip the soft-delete flag instead of hard deleting from the frontend.
...
- **VIPBillion tables**: include `isDelete boolean NOT NULL DEFAULT false` on most CRUD-managed business tables, plus active-row filters in every read path
