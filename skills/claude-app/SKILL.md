...
- **MODULE OPEN HYGIENE** — when a create drawer, edit drawer, modal, or CRUD module opens, initialize it in a clean state: reset inputs/selects to their defaults, clear stale edit-history values unless the current mode intentionally loads an existing record, and avoid leaking previously stored form state into a fresh create flow.
- **SCROLL RESET ON OPEN** — every module open should start scrolled to the top before focus lands, so reopened drawers/modals do not resume mid-scroll from the prior session.
...
Soft-delete rule:
- if the table uses `deleted_at` or equivalent soft delete, slug uniqueness must apply to active rows only
- app-side checks must filter deleted rows
- DB must use a partial unique index on active rows only
- if create says slug is available but submit fails with duplicate constraint, suspect an old global unique index/constraint first
- detailed reusable pattern: `skills/claude/BLOG_TITLE_DUPLICATE_CHECK_PATTERN.md` section `Soft-Delete Slug Reuse Pattern (Critical)`
- adopt this as the default implementation for future slug-enabled CRUD modules unless the user explicitly asks for a different slug lifecycle
...
## Attachment Album Pattern (2026-06-11)

For Vben Admin + Supabase projects where multiple CRUD modules upload images, the admin must include one shared **Attachment Album** module that acts as the final registry and controller for uploaded files.

### Required behavior
- Every successful storage upload must also insert a row into a business table such as `schema.attachments`.
- The album page must read from `attachments`, not directly from `storage.objects`.
- Existing uploaded images from current content tables must be backfilled into `attachments` with a migration.
- The album is the single admin place to:
  - browse uploaded images
  - preview and copy URLs
  - soft delete / restore
  - permanently delete files
- When a file is permanently deleted from Attachment Album, the system must also clear all active references in other business tables so those pages no longer point at a missing file.

### Database contract
- Create `schema.attachments` with snake_case fields:
  - `id`
  - `project_id`
  - `storage_path`
  - `original_name`
  ...
  - `updated_at`
- Add:
  - unique active index on `(project_id, storage_path)` where `is_delete = false`
  - updated_at trigger
  - authenticated CRUD RLS/grants
- Add a `clear_attachment_references(p_storage_path text)` RPC in the business schema:
  - scan every image-bearing table/column used by the project
  - null direct image columns that match the path
  - remove the path from json/jsonb image arrays
  - update `updated_at`
- Make this RPC `SECURITY DEFINER` and grant execute to `authenticated`.

### Upload integration
- The upload utility must do 2 steps:
  1. upload to Supabase Storage
  2. insert the uploaded file into `attachments`
- If the business schema is snake_case, insert snake_case keys from the frontend upload utility. Do not assume camelCase column names.

### Backfill rule
- Add a migration that unions all current storage-backed paths from image columns across modules, for example:
  - slideshows
  - services
  - attractions
  - vehicles
  - news / blog / insights
  - reviews / avatars
- Exclude hardcoded website static assets like `images/...` when those are not storage-managed.
- Make the backfill idempotent with `ON CONFLICT DO NOTHING`.

### UI / store rule
- Attachment Album store may map snake_case DB rows into camelCase view models for Vue.
- The list page should default to newest first.
- The page should expose:
  - All / Active / Deleted filters
  - preview
  - copy URL
...
### Implementation note
- If the project already has working module uploads but Attachment Album is empty, suspect the missing layer in this order:
  1. no `attachments` table
  2. upload utility does not insert attachment rows
  3. old images were never backfilled
  4. album store/view assumes wrong casing
  5. delete RPC does not clear references
