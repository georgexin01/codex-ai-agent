...
When the user wants a mobile app to closely replicate a screenshot or reference UI, use **Hierarchical Replica Chunking** before implementation.

Meaning:

- break the screen into nested visual chunks
- each `c` is not just a single element
- each `c` can be one grouped visual unit containing structure plus CSS notes

Example:

`A = H + (C1 + (C1a + C1b + C1c)) + C2 + C3 + C4 + C5`
...
- the user wants a same-looking replica
- section-level accuracy matters
- screenshot-derived image composition matters

Important:

- do not treat `c` as only one tag
- do not solve image mismatch by bad crop-only fitting
- if the image composition is critical, prefer a newly generated or newly selected ratio-matched image over destructive cropping

## Important Rule: Persistent Reference Monitoring

When the user provides a screenshot or uploaded image and wants a replica, clone, or exact same design, the reference image must remain an active monitoring target for the whole task.
...
Do not rely on markdown notes alone after the first read.

Use together:

- reference image monitoring
- chunk notes
- design contract
