...
## Replica Chunking Addendum

When the user wants a website to closely replicate a screenshot or reference layout, use **Hierarchical Replica Chunking** before implementation.

Meaning:

- decompose the page into nested visual chunks
- each `c` is one smallest meaningful visual replica unit
- one `c` may contain grouped HTML structure plus visual styling notes
...
- typography rules
- icon placement
- image ratio and focal framing
- overlay or badge behavior

Use this when:

- the user says replica, clone, same look, match exactly, or screenshot-based redesign
- the design has dense visual hierarchy
- high-fidelity matching matters more than generic redesign

Important:

- do not treat `c` as only one DOM element
- document chunk breakdown in markdown for high-accuracy tasks
- if image composition mismatch happens, prefer a better-matched or generated image at the correct ratio instead of forcing poor cropping
...
## Article / News Detail Navigation Pattern (2026-06-11)

When a website has article, news, blog, or newspaper-style detail pages, prefer a **previous/current/next** navigation pattern backed by a single database fetch instead of loading an arbitrary list and trimming it in PHP.

Mandatory behavior:

1. **Use one DB call** for the detail context:
   - current article
   - previous article
   - next article
2. For Supabase/Postgres, prefer a SQL function / RPC using window functions (`row_number()`, `lag()`, `lead()`) or equivalent ordered-neighbor logic.
3. Ordering must follow the website's real article order source:
   - `sort_order DESC NULLS LAST`
   - then `published_at DESC NULLS LAST`
   - then `created_at DESC NULLS LAST`
   unless project truth requires another ordering contract.
4. Do **not** duplicate the current article inside the related-navigation section.
5. Edge cases:
   - latest article → previous missing, next exists
   - oldest article → next missing, previous exists
   - only one article → both missing
6. Rendering rule:
   - render only the available neighbor cards
   - if both neighbors are missing, hide the whole section
   - never show fake empty placeholders or "No data" cards for this pattern
7. Preferred UI when image support is unnecessary:
   - use **text-first previous / next cards**
   - previous card floats left with left arrow
   - next card floats right with right arrow
   - previous text aligns left, next text aligns right
   - subtle arrow hover motion is preferred over large decorative effects
8. Only use image cards when the page intentionally needs image-led editorial navigation.
9. Keep the navigation block visually lighter than the main article body and do not overpower the content.

Recommended implementation shape:

- SQL / Supabase RPC returns:
  - `previous_item`
  - `current_item`
  - `next_item`
- PHP / template layer renders:
  - `Previous News`
  - `Next News`
  - hides the wrapper if neither exists

This pattern is now the default for future `.php` website article/news detail pages unless the user explicitly asks for a carousel, image grid, or multi-card related-post layout.

## Important Rule: Persistent Reference Monitoring

When the user provides a screenshot or uploaded image and wants a website replica, clone, copy, or exact same design, the reference image must remain an active monitoring target for the whole task.
...
3. compare each changed chunk back to the reference image
4. continue this loop until the task ends

Do not downgrade to markdown-only interpretation after the first pass.
