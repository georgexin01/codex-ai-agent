...
# Detect exact trigger collisions across the primary PULSE map and semantic router.
...
if (Test-Path -LiteralPath $semanticRouterPath) {
  foreach ($hit in (Select-String -LiteralPath $semanticRouterPath -Pattern '^\s*\|\s*`([^`]+)`\s*\|\s*`([^`]+)`')) {
    $triggerEntries.Add([pscustomobject]@{ trigger = $hit.Matches[0].Groups[1].Value.Trim().ToLowerInvariant(); target = $hit.Matches[0].Groups[2].Value.Trim(); source = "skill_path_router.md" })
  }
}
